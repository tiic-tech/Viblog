...
async function main() {
  ...
  const client = new Client(
    { name: 'viblog-test-client', version: '1.0.0' },
    { capabilities: {} }
  )
  ...
  try {
    ...
    const sessionData = JSON.parse((sessionResult.content as Array<{ type: string; text: string }>)[0].text)
      ...
      arguments: {
        ...
        fragments: [
          ...
          },
          {
            ...
            content:
              'User asked: "Claude Code captures internally. OpenClaw saves locally. Viblog shares with everyone. Does this positioning hold up?"',
            sequence_number: 2,
          },
        ],
      },
    ...
    const uploadData = JSON.parse((uploadResult.content as Array<{ type: string; text: string }>)[0].text)
    ...
    const publishResult = await client.callTool({
      ...
      arguments: {
        ...
        content: ARTICLE_CONTENT,
        excerpt:
          'A critical analysis of Viblog ecosystem positioning and strategic recommendations for brand development.',
        visibility: 'private',
      },
    })

    const publishData = JSON.parse((publishResult.content as Array<{ type: string; text: string }>)[0].text)
    console.log('\n=== Article Published! ===')
