...
### Custom Agents YAML Frontmatter Fix (2026-03-19)

**Issue:** 6 custom agents in `~/.claude/agents/` were not appearing in `/agents` command.

**Root Cause:** YAML frontmatter `description` fields contained colons followed by numbered lists (e.g., `for: 1)`), which YAML interprets as nested mappings, causing parse errors:
```
...
**Problematic Pattern:**
```yaml
description: Trigger this agent for: 1) PR approval, 2) Refactoring...
```

The `for:` was parsed as a nested key instead of part of the description string.

**Solution:** Used YAML block scalar syntax (`>`) for multi-line descriptions:
```yaml
description: >
  High-authority technical executive for architectural auditing.
  Trigger for PR approval, refactoring plans, security audits.
```

**Files Fixed:**
| File | Status |
|------|--------|
| `chief-technology-officer.md` | Fixed |
| `chief-ui-officer.md` | Fixed |
| `develop-reviewer.md` | Fixed |
| `design-reviewer.md` | Fixed |
| `ui-concept-generator.md` | Fixed |
| `ui-prompt-builder.md` | Fixed |

**Lesson Learned:**
- YAML interprets `key: value:` as nested mappings
- Use block scalars (`>` or `|`) for multi-line strings with special characters
- `>` folds newlines into spaces (single line output)
- `|` preserves newlines (literal block)

**Verification:** All 6 agents now parse correctly with Python YAML validator.
